...
public:
                      ...
                      bool compareOrder = false) -> bool
    {
        ...
        if (compareOrder) {
            return std::ranges::equal(group1, group2);
        }

        return std::is_permutation(group1.begin(), group1.end(), group2.begin());
    }
};
...
public:
    ...
    static auto group(std::string_view id, std::string_view elements) -> std::string
    {
        if (elements.empty()) {
            return "";
        }
