...
public:
  static auto angle(double radians) -> double { return radians * 180.0 / PI; }
...
public:
  ...
  void operator+=(Point point) { sum(point.X.value, point.Y.value); }

  void operator+=(double value) { sum(value, value); }
  ...
  void operator-=(const Point point) { sum(-point.X.value, -point.Y.value); }
  ...
  void operator*=(const Point point) { multiply(point.X.value, point.Y.value); }

  void operator*=(double value) { multiply(value, value); }

  auto operator==(const Point point) const -> bool { return equal(point); }
...
public:
  ...
  auto operator==(const Base &polygon) -> bool { return equal(polygon); }
...
public:
  ...
  static auto group(std::string id, const std::string &elements)
      -> std::string {
    id = id.empty() ? "<g>\n" : "<g id=\"" + id + "\" >\n";
...
public:
  struct RGBA {
    ...
    auto empty() const -> bool { return R == 0 && G == 0 && B == 0 && A == 0; }
